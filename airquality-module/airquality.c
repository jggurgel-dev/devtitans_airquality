...
static struct usb_device *airquality_device;        // Referência para o dispositivo USB
...
// Cria e registra o driver do airquality no kernel
static struct usb_driver airquality_driver = {
    .name        = "airquality",     // Nome do driver
...
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    ...
    // Cria arquivos do /sys/kernel/airquality/*
...
static void usb_disconnect(struct usb_interface *interface) {
    ...
    if (sys_obj) kobject_put(sys_obj);      // Remove os arquivos em /sys/kernel/airquality
...
static int usb_send_cmd(char *cmd, int param) {
    ...
    while (retries > 0) {
        // Lê dados da USB
        ret = usb_bulk_msg(airquality_device, usb_rcvbulkpipe(airquality_device, usb_in), usb_in_buffer, min(usb_max_size, MAX_RECV_LINE), &actual_size, HZ*1000);
        if (ret) {
            printk(KERN_ERR "AirQuality: Erro ao ler dados da USB (tentativa %d). Codigo: %d\n", ret, retries--);
            continue;
        }
        ...
        for (i=0; i<actual_size; i++) {
            ...
            if (usb_in_buffer[i] == '\n') {  // Temos uma linha completa
                ...
                printk(KERN_INFO "AirQuality: Recebido uma linha: '%s'\n", recv_line);
                ...
                if (!strncmp(recv_line, resp_expected, strlen(resp_expected))) {
                    printk(KERN_INFO "AirQuality: Linha eh resposta para %s! Processando ...\n", cmd);
